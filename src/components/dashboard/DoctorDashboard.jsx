...
export function DoctorDashboard({ userName = "Dr. Smith" }) {
  ...
  const stats = [
    { label: "Appointments Today", value: "8", icon: CalendarDays, color: "#4F46E5" },
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-12 mb-8">
        {/* Today's Schedule Section (using Activity List format) */}
        <motion.div
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          ...
          <div className="bg-white rounded-xl border border-[#E5E7EB] shadow-[0_2px_8px_rgba(0,0,0,0.04)] divide-y divide-[#E5E7EB]">
            {todaysSchedule.map((appt, index) => (
              ...
              >
                ...
                >
                  <appt.icon size={20} style={{ color: '#4F46E5' }} />
                </div>

                <div className="flex-1 min-w-0">
                  <h3 style={{ fontSize: '15px', fontWeight: 600, color: '#111827' }}>
                    {appt.title} <span className="text-sm font-normal text-[#6B7280]">({appt.status})</span>
                  </h3>
        ...
        {/* My Patients Section (using DoctorCard format inside a vertical list container) */}
        <motion.div
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
        ...
        >
          ...
          <div className="flex flex-col gap-4">
            {myPatients.map((patient) => (
              ...
              >
                <div className="w-12 h-12 bg-gradient-to-br from-[#10B981] to-[#047857] rounded-xl flex items-center justify-center text-white flex-shrink-0" style={{ fontSize: '15px', fontWeight: 600 }}>
                  {patient.avatar}
                </div>
